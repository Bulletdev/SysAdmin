...
echo -e "${GREEN} Starting SysAdmin Security Testing Lab${NC}"
echo "=========================================="
echo ""
...
echo -e "${YELLOW} Starting SysAdmin Rails application...${NC}"
...
for i in {1..30}; do
    if curl -sf http://localhost:4040 > /dev/null 2>&1; then
        echo -e "${GREEN}✓ App is responding on :4040${NC}"
        break
    fi
    if [ $i -eq 30 ]; then
...
echo "  • Rails (web):     http://localhost:4040"
echo "  • ZAP Web UI:      http://localhost:8087/zap"
echo "  • ZAP API:         http://localhost:8097"
echo ""
echo "🔍 Running Containers:"
docker ps --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}" | grep sysadmin
echo ""
echo " Next Steps:"
echo "  1. Run security scans:"
echo "     ./security_tests/run-security-scans.sh"
echo "     # Windows PowerShell: ./security_tests/run-security-scans.ps1"
echo ""
